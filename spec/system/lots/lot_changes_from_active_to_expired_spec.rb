...
describe 'Lote muda de ativo para expirado' do
  it 'quando a data de limite para lance é ultrapassada' do
    ...
    admin = User.create!(name: 'Ricardo', email: 'ricardo@leilaodogalpao.com.br', registration_number: '70535073607',
                         password: 'password')
    user = User.create!(name: 'Fernanda', email: 'fernanda@exemplo.com.br', registration_number: '59310423005',
                        password: 'password')
    travel_to Time.current - 2.week do
      ...
      Bid.create!(value: 140, user_id: user.id, lot_id: lot.id, created_at: 3.day.from_now)
    end
    # Act
    login_as(admin)
    visit root_path
