...
class Lot < ApplicationRecord
  ...
  def code_format_validation
    ...
    return unless !validate_characters(characters) || !validate_numbers(numbers)
  ...
  def bid_value_to_beat
    ...
    last_bid = bids.last
    if last_bid&.value && last_bid.value >= min_value
      min_value = last_bid.value + minimum_bid_difference
    elsif last_bid&.value && last_bid.value < min_value
      false
    end
    min_value
  end
  ...
  def date_range_validation
    return unless start_date.present? && limit_date.present? && start_date >= limit_date

    errors.add(:start_date,
               'deve ser anterior à data limite')
  end
  ...
  def successful_bid_data
    bl = bids.last
    if bl
      "#{bl.user.name} | #{bl.user.email} | R$#{bl.value},00 | Lote #{code.upcase} | Lote expirado em #{I18n.l(
        limit_date, format: '%d de %B de %Y'
      )}"
    else
